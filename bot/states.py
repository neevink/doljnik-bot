...
class StateYDBStorage(StateStorageBase):
    ...
    def set_data(self, chat_id, user_id, key, value):
        ...
        """
        if db_model.get_state(self.pool, user_id, chat_id) is None:
            return False

        full_state = db_model.get_state(self.pool, user_id, chat_id)
        full_state["data"][key] = value

        db_model.set_state(self.pool, user_id, chat_id, full_state)
        return True
    ...
    def delete_state(self, chat_id, user_id):
        ...
        db_model.clear_state(self.pool, user_id, chat_id)
        return True
    ...
    def get_state(self, chat_id, user_id):
        states = db_model.get_state(self.pool, user_id, chat_id)
        if states is None:
            return None
